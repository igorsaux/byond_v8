...
async fn execute_with_timeout(
    ...
    let isolate = rt.v8_isolate().thread_safe_handle();
...
pub async fn execute_code(rt: &mut dyn Runtime, code: &str) -> String {
    let rt = rt.runtime_mut();
    let value = execute_with_timeout(rt, code, Duration::from_secs(2)).await;
...
mod tests {
    ...
    async fn execute_js() {
        ...
        let result = execute_code(&mut rt, "let a = [1, 2, 3]; a.map(i => i * 2)").await;
    ...
    async fn execute_infinite_loop() {
        ...
        let result = execute_code(&mut rt, "while (true) {}; 1").await;
